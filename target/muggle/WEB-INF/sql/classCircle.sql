...
INSERT INTO `Message` VALUES ('M000000000000003','A','U000000000000006','家长的','','2017-04-20 19:34:04');
...
INSERT INTO `Notice` VALUES ('N000000000000001','U000000000000001','C000000000000001','及时交作业','2017-04-25 13:22:17'),('N000000000000002','U000000000000001','C000000000000001','第二个公告','2017-05-07 17:46:40');
...
BEGIN
    ...
    DECLARE id CHAR(16);
    DECLARE cursor_tmp CURSOR FOR SELECT n_id FROM tmp_notice;
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET error_flag = -1;
    START TRANSACTION ;
    SET counts = (SELECT count(s_id) FROM Student WHERE s_class = in_class);
    CREATE TEMPORARY TABLE IF NOT EXISTS tmp_notice(
      n_id CHAR(16) PRIMARY KEY NOT NULL ,
      n_user CHAR(16) NOT NULL ,
      n_content TEXT NOT NULL ,
      n_time DATETIME NOT NULL ,
      t_header TEXT,
      name VARCHAR(20),
      t_power BIT NOT NULL ,
      signed INT,
      counts INT
    );
    TRUNCATE TABLE tmp_notice;
    INSERT INTO tmp_notice SELECT n_id,n_user,n_content,n_time,t_header,Subject.name,t_power,NULL ,counts FROM Notice JOIN User ON Notice.n_user = User.u_id LEFT JOIN Teacher ON User.u_teacher = Teacher.t_id JOIN Subject ON Teacher.t_subject = Subject.id WHERE n_class = in_class ORDER BY n_time ASC;
    SET cnt = (SELECT count(n_id) FROM tmp_notice);
    OPEN cursor_tmp;
    WHILE var < cnt DO 
      FETCH cursor_tmp INTO id;
      UPDATE tmp_notice SET signed = (SELECT NoticeSign.id FROM NoticeSign WHERE NoticeSign.notice = id) WHERE tmp_notice.n_id = id;
      SET var = var + 1;
    END WHILE;
    CLOSE cursor_tmp;
    SELECT * FROM tmp_notice;
    IF error_flag = -1 THEN
      SET resultCode = -1;
      ROLLBACK ;
    ELSE
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `loadMessageCount` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `loadMessageCount`(
  IN in_class CHAR(16),
  IN in_time DATETIME,
  out resultCode int
)
BEGIN
    ...
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET error_flag = -1;
    START TRANSACTION ;
    CREATE TEMPORARY TABLE IF NOT EXISTS tmp_messageCount(
      m_id CHAR(16) PRIMARY KEY NOT NULL,
      m_time DATETIME NOT NULL
    );
    TRUNCATE TABLE tmp_messageCount;
    INSERT INTO tmp_messageCount SELECT m_id,m_time FROM Message JOIN User ON Message.m_user = User.u_id LEFT JOIN Teacher ON User.u_teacher = Teacher.t_id JOIN TC ON Teacher.t_id = TC.tc_teacher JOIN Subject ON Teacher.t_subject = Subject.id
    WHERE TC.tc_class = in_class;
    INSERT INTO tmp_messageCount SELECT m_id,m_time FROM Message JOIN User ON Message.m_user = User.u_id LEFT JOIN Parent ON User.u_parent = Parent.p_id JOIN PS ON Parent.p_id = PS.ps_parent JOIN Student ON PS.ps_student = Student.s_id
    WHERE s_class = in_class;
    SELECT count(m_id) FROM tmp_messageCount WHERE unix_timestamp(m_time) >= unix_timestamp(in_time);
    IF error_flag = -1 THEN 
      SET resultCode = -1;
      ROLLBACK ;
    ELSE
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `loadNoticeCount` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `loadNoticeCount`(
  IN in_class CHAR(16),
  IN in_time DATETIME,
  out resultCode int
)
BEGIN
    ...
    DECLARE CONTINUE HANDLER FOR SQLEXCEPTION SET error_flag = -1;
    START TRANSACTION ;
    SELECT count(n_id) FROM Notice JOIN User ON Notice.n_user = User.u_id LEFT JOIN Teacher ON User.u_teacher = Teacher.t_id JOIN Subject ON Teacher.t_subject = Subject.id LEFT JOIN NoticeSign ON Notice.n_id = NoticeSign.notice WHERE n_class = in_class AND unix_timestamp(n_time) >= unix_timestamp(in_time);
    IF error_flag = -1 THEN 
      SET resultCode = -1;
      ROLLBACK ;
    ELSE 
...
-- Dump completed on 2017-05-08 10:09:55
